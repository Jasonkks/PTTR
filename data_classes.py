...
class Box:
    ...
    def transform(self, transf_matrix, rtol=1e-6, atol=1e-6):
        center = np.array([*self.center, 1])
        transformed = transf_matrix @ center[None, ...].T
        self.center = transformed.T[0, :3]
        self.orientation = self.orientation* Quaternion(matrix = transf_matrix[0:3,0:3], rtol=rtol, atol=atol)
